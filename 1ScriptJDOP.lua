-- [[ {"message":"Script error not found!"} ]]
...
local function _0xCP()
    if isfile(_0xFN.."/".._0xFT) then
        ...
        local nivel = tonumber(n) or 1
        local tiempo = tonumber(t) or 0
        
        -- LÓGICA DE 24 HORAS (86400 SEGUNDOS)
        if os.time() - tiempo >= 86400 and tiempo ~= 0 then
            nivel = nivel + 1
            if nivel > 31 then nivel = 1 end
            -- Guardamos el nuevo nivel inmediatamente
            writefile(_0xFN.."/".._0xFT, tostring(nivel)..","..tostring(os.time()))
        end
        return nivel, tiempo
    end
    return 1, 0
...
UnlockBtn.MouseButton1Click:Connect(function()
    ...
    if input == keyCorrecta then
        ...
        -- Si es la primera vez que entra, guardamos el tiempo inicial
        local tGuardado = _0xUA
        if tGuardado == 0 then tGuardado = os.time() end
        
        _0xGP(_0xNA, tGuardado)
        task.wait(1)
        ScreenGui:Destroy()
        _0xEX()
    else
